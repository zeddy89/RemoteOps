...
program
    ...
    try {
        ...
        const statusColor = updateResult.status === 'success' ? chalk.green :
...
async function promptForMissingConfig(options) {
    const questions = [];
    if (!options.host) {
        questions.push({
            type: 'input',
    ...
    if (!options.key && !options.password) {
        ...
        const authAnswer = await inquirer.prompt([questions.pop()]);
    ...
    if (authMethod === 'config' || options.config) {
        ...
        if (keyFiles.length > 0) {
            ...
            const keyAnswer = await inquirer.prompt([questions.pop()]);
            if (keyAnswer.key === 'custom') {
                ...
                const { hasPassphrase } = await inquirer.prompt([questions.pop()]);
