...
export class Troubleshooter {
  ...
  async analyzeDiagnostics(results: DiagnosticResult[]): Promise<TroubleshootingAction[]> {
    const actions: TroubleshootingAction[] = [];
  ...
  async executeAction(action: TroubleshootingAction): Promise<string> {
    return action.execute();
  }

  async getCustomActionForIssue(issue: string): Promise<TroubleshootingAction[]> {
    const actions: TroubleshootingAction[] = [];
    
    // Common user-reported issues and their troubleshooting commands
    const issuePatterns: [RegExp, TroubleshootingAction[]][] = [
      [
        /slow/i, 
        [
          {
