...
export type OSType = 'windows' | 'linux' | 'unix' | 'unknown';
...
export class OSDetector {
  private static cache = new Map<string, OSInfo>();

  /**
   * Detect the operating system of a remote host
   */
  static async detectOS(client: SSHClient, host: string): Promise<OSInfo> {
    // Check cache first
   ...
   */
  private static async tryWindowsDetection(client: SSHClient): Promise<OSInfo | null> {
    try {
   ...
   * Try to detect Linux
   */
  private static async tryLinuxDetection(client: SSHClient): Promise<OSInfo | null> {
    try {
   ...
   */
  private static async tryUnixDetection(client: SSHClient): Promise<OSInfo | null> {
    try {
   ...
   */
  static formatCommand(osInfo: OSInfo, command: string): string {
    switch (osInfo.shell) {
      case 'powershell':
   ...
   */
  static getSystemInfoCommand(osInfo: OSInfo): string {
    switch (osInfo.type) {
      case 'windows':
        return 'systeminfo';
   ...
   */
  static getDiskSpaceCommand(osInfo: OSInfo): string {
    switch (osInfo.type) {
      case 'windows':
   ...
   */
  static getMemoryCommand(osInfo: OSInfo): string {
    switch (osInfo.type) {
      case 'windows':
   ...
   */
  static getCPUCommand(osInfo: OSInfo): string {
    switch (osInfo.type) {
      case 'windows':
   ...
   */
  static getTopProcessesCommand(osInfo: OSInfo): string {
    switch (osInfo.type) {
      case 'windows':
