...
export class SystemMonitor {
  constructor(private client: SSHClient, private osInfo: OSInfo) {}

  /**
   * Get comprehensive system metrics
   */
  async getSystemMetrics(host: string): Promise<SystemMetrics> {
  ...
  private async getWindowsCPUMetrics(): Promise<SystemMetrics['cpu']> {
  ...
  private async getLinuxCPUMetrics(): Promise<SystemMetrics['cpu']> {
  ...
  private async getWindowsMemoryMetrics(): Promise<SystemMetrics['memory']> {
  ...
  private async getLinuxMemoryMetrics(): Promise<SystemMetrics['memory']> {
  ...
  private async getWindowsDiskMetrics(): Promise<SystemMetrics['disk']> {
    ...
    const disks: SystemMetrics['disk'] = [];
  ...
  private async getLinuxDiskMetrics(): Promise<SystemMetrics['disk']> {
  ...
  private async getWindowsNetworkMetrics(): Promise<SystemMetrics['network']> {
  ...
  private async getLinuxNetworkMetrics(): Promise<SystemMetrics['network']> {
  ...
  private async getWindowsTopProcesses(limit: number): Promise<ProcessInfo[]> {
  ...
  private async getLinuxTopProcesses(limit: number): Promise<ProcessInfo[]> {
   ...
   */
  async monitorProcess(pid: number): Promise<ProcessInfo | null> {
    try {
      if (this.osInfo.type === 'windows') {
   ...
   * Get system uptime
   */
  async getSystemUptime(): Promise<{ uptime: string; bootTime: Date | null }> {
    try {
      if (this.osInfo.type === 'windows') {
        const command = `powershell "
          $os = Get-WmiObject Win32_OperatingSystem
