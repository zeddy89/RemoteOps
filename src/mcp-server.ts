...
class RemoteOpsServer {
  ...
  private createSSHConfig(args: any): SSHConnectionConfig {
    return {
      host: args.host,
      port: args.port || 22,
  ...
  private async handleSshMultiDiagnose(args: any) {
    const results = [];
  ...
  private async handleHypervListVMs(args: any) {
    const config = this.createSSHConfig(args);
  ...
  private async handleSshMultiCommand(args: any) {
    ...
    const executeOnServer = async (server: any): Promise<ServerResult> => {
      ...
      try {
        ...
        const result = await Promise.race([
          ...
          new Promise((_, reject) => 
            setTimeout(() => reject(new Error(`Command timeout after ${timeout}s`)), timeout * 1000)
          )
        ]) as any;
